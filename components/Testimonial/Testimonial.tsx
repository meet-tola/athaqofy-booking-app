...
export default function Testimonials() {
  ...
  const getTextItemsPerSlide = () => {
    if (windowWidth < 768) return 1;
    if (windowWidth < 1024) return 2;
    return 3;
  };

  const getAudioItemsPerSlide = () => {
    if (windowWidth < 768) return 1;
    return 2;
  };

  const [textItemsPerSlide, setTextItemsPerSlide] = useState(
    getTextItemsPerSlide()
  );
  const [audioItemsPerSlide, setAudioItemsPerSlide] = useState(
    getAudioItemsPerSlide()
  );
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setTextItemsPerSlide(getTextItemsPerSlide());
      setAudioItemsPerSlide(getAudioItemsPerSlide());
    };
  ...
  const prevTextSlide = () => {
    setCurrentTextSlide(
      (prev) => (prev - 1 + totalTextSlides) % totalTextSlides
    );
  };
  ...
  const prevAudioSlide = () => {
    setCurrentAudioSlide(
      (prev) => (prev - 1 + totalAudioSlides) % totalAudioSlides
    );
  };
              ...
              {Array.from({ length: totalTextSlides }).map((_, slideIndex) => (
                <div key={slideIndex} className="w-full flex-shrink-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-1">
                    {testimonials
                      .slice(
                        slideIndex * textItemsPerSlide,
                        (slideIndex + 1) * textItemsPerSlide
                      )
                      .map((testimonial, idx) => (
                        <Card
                          key={idx}
              ...
              {Array.from({ length: totalAudioSlides }).map((_, slideIndex) => (
                <div key={slideIndex} className="w-full flex-shrink-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-1">
                    {testimonials
                      .slice(
                        slideIndex * audioItemsPerSlide,
                        (slideIndex + 1) * audioItemsPerSlide
                      )
                      .map((testimonial, idx) => {
                        const absoluteIndex =
                          slideIndex * audioItemsPerSlide + idx;
                        return (
                          <Card
                            key={idx}
